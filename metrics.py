...
def com_accuracy(loader, model):
    model.eval()
    correct = 0
    total = 0
    for images, _, com_label in loader:
        images = images.to(device)
        com_label = com_label.to(device)
        output = model(images)
        outputs = F.softmax(output, dim=1)
        _, com_hot = torch.max(com_label.data, 1)
        _, pred = torch.max(outputs.data, 1)
        # print(com_hot, pred)
        total += images.size(0)
        correct += (pred == com_hot).sum().item()
